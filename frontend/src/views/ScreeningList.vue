<template>
  <div>
    <h1 class="mb-6 text-2xl font-bold text-stone-800 sm:text-3xl">
      Screenings
    </h1>

    <p v-if="loading" class="flex items-center gap-2 text-stone-500">
      <span
        class="inline-block h-5 w-5 animate-spin rounded-full border-2 border-amber-500 border-t-transparent"
      />
      Loading...
    </p>
      ...
      >
        <div class="p-5">
          <router-link :to="`/screenings/${s.id}`" class="group block">
            <strong
              class="block text-lg font-semibold text-stone-800 group-hover:text-amber-600"
            >
              {{ s.movie_name }}
            </strong>
            <span class="mt-2 block text-sm text-stone-500">{{
              formatDate(s.screen_at)
            }}</span>
            <span
              class="mt-1 inline-block rounded-full bg-stone-200 px-3 py-0.5 text-xs text-stone-600"
            >
              {{ s.rows }}×{{ s.cols }} seats
            </span>
          </router-link>
        ...
        >
          <p
            v-if="detailsLoading === s.id"
            class="flex items-center gap-2 py-2 text-sm text-stone-500"
          >
            <span
          ...
          <template v-else-if="details[s.id]">
            <div
              v-if="details[s.id].locked.length || details[s.id].booked.length"
              class="space-y-4"
            >
              <!-- ที่นั่งถูกล็อก -->
              <div v-if="details[s.id].locked.length">
                <h4 class="mb-2 text-sm font-semibold text-amber-700">
                  ที่นั่งถูกล็อก
                </h4>
                <div class="overflow-x-auto rounded-lg border border-stone-200">
                  <table class="w-full min-w-[320px] text-left text-sm">
                    <thead class="bg-stone-100">
                      <tr>
                        <th class="px-3 py-2 font-medium text-stone-700">
                          แถว-คอลัมน์
                        </th>
                        <th class="px-3 py-2 font-medium text-stone-700">
                          ผู้ล็อก (User ID)
                    ...
                    <tbody class="divide-y divide-stone-100">
                      <tr
                        v-for="x in details[s.id].locked"
                        :key="`lock-${x.row}-${x.col}`"
              ...
              <div v-if="details[s.id].booked.length">
                <h4 class="mb-2 text-sm font-semibold text-green-700">
                  ที่นั่งจองแล้ว
                </h4>
                <div class="overflow-x-auto rounded-lg border border-stone-200">
                  <table class="w-full min-w-[280px] text-left text-sm">
                    ...
                    <tbody class="divide-y divide-stone-100">
                      <tr
                        v-for="x in details[s.id].booked"
                        :key="`book-${x.row}-${x.col}`"
                        class="text-stone-600"
                      >
                        <td class="px-3 py-2">
                          {{ x.row + 1 }}-{{ x.col + 1 }}
            ...
            <p v-else class="py-2 text-sm text-stone-500">
              ยังไม่มีที่นั่งถูกล็อกหรือจอง
            </p>
          </template>
        </div>
      </li>
    ...
    <p
      ...
      class="mt-8 rounded-lg border border-stone-200 bg-stone-50 p-6 text-center text-stone-500"
    >
...
function connectWs(screeningId) {
  ...
  ws.onmessage = async (ev) => {
    try {
      ...
      if (
        msg.type === "SEAT_UPDATE" &&
        msg.payload &&
        expandedId.value === screeningId
      ) {
        const data = await getSeatDetails(screeningId).catch(() => ({
          locked: [],
          booked: [],
        }));
        details.value[screeningId] = {
          locked: data.locked || [],
          booked: data.booked || [],
        };
      }
